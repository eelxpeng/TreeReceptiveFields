...
from lib.makeLayer import makeLayerMaskedDAEFCMI

class TreeMIConvNetFC:
    ...
    def learn_structure(self, trainX, validX, num_class, kernel_stride, fcwidths, corrupt=0.5,
        ...
        for l in range(num_layers):
            ...
            fw = fcwidths[l]
            if l==0:
                layer = makeLayerMaskedDAEFCMI(tr_x, va_x, corrupt=0.5, kernel_size=k, stride=s, fcwidth=fw,
                    lr=1e-3, batch_size=batch_size, epochs=10, loss_type=loss_type)    
            else:
                layer = makeLayerMaskedDAEFCMI(tr_x, va_x, corrupt=0.5, kernel_size=k, stride=s, fcwidth=fw,
                    lr=1e-3, batch_size=batch_size, epochs=10)
            layer.eval()
