...
logging.info("l1 wd=%.5f" % (args.wd))

if args.dataset=="tox":
    label_name = ['0','1']
    from lib.Tox21_Data import read
    trainX, trainY, validX, validY, testX, testY = read("dataset/tox21/", target=args.target, valid_num=500)
...
def loss_l1(model):
    ...
    for param in model.parameters():
        target = torch.zeros(param.size())
        if use_cuda:
            target.cuda()
        target = Variable(target)
        l1_loss=F.l1_loss(param, target=target, size_average=False)
        reg_loss += l1_loss
    return reg_loss
...
def train(net):
    ...
    for epoch in range(args.epochs):
        ...
        for batch in range(batchnum):
            ...
            output = net(batchX)
            loss = criterion(output, batchY) + args.wd*loss_l1(net)
            loss.backward()
            optimizer.step()
...
logging.info("Sparsity: %f" % check_sparsity(fnn, thresh=1e-3))

if not os.path.isdir('checkpoint'):
    os.mkdir('checkpoint')
save_path='./checkpoint/ckpt-'+args.name+'.t7'
torch.save(fnn, save_path)
