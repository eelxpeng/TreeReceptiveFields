...
x_tr_t, y_tr_t, x_valid_t, y_valid_t, x_te_t, y_te_t = read("./dataset/tox21/", target=args.target)
...
if os.path.exists("mask.pkl"):
    mask = pickle.load(open("mask.pkl","rb"))
else:
    mask = genMask(train_data, todiscretize=True, bins=2, kernel_size=3, stride=2)
...
optimizer = optim.Adam(filter(lambda p: p.requires_grad, maskedLinear1.parameters()), lr=args.lr)
...
if not os.path.isdir('checkpoint'):
    os.mkdir('checkpoint')
torch.save(state, './checkpoint/ckpt-pretrain.t7')

print("=========Classify============")
for t in range(12):
    print("Training for Task #%d..." % t)
    pretrained = torch.load('./checkpoint/ckpt-pretrain.t7')
    maskedLinear1 = pretrained['maskedLinear1']
    maskedLinear2 = pretrained['maskedLinear2']
    x_tr_t, y_tr_t, x_valid_t, y_valid_t, x_te_t, y_te_t = read("../datasets/tox21/", target=t)
    trainset = Dataset(x_tr_t, y_tr_t)
    validset = Dataset(x_valid_t, y_valid_t)
    testset = Dataset(x_te_t, y_te_t)
    trainloader = torch.utils.data.DataLoader(
